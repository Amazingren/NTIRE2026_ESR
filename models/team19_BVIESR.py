...
def window_partition(x, window_size):
    ...
    H_div, W_div = H // window_size_h, W // window_size_w
    x = x.view(B, H_div, window_size_h, W_div, window_size_w, C)
...
class SCC(nn.Module):
    ...
    def channel_self_correlation(self, q, v):
        ...
        x = (corr_map @ v_drop.transpose(-2, -1)).transpose(1, 2).reshape(B, L, -1)
    ...
    def forward(self, x):
        ...
        # qv splitting
        B, L, C = qv.shape
        qv = qv.view(B, L, 2, C // 2).permute(2, 0, 1, 3).contiguous()
        q, v = qv[0], qv[1]  # B, L, C//2
        ...
        x = self.proj_drop(self.proj(x_channel))
...
class HierarchicalTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, base_win_size, window_size,
        ...
        if (window_size[0] > base_win_size[0]) and (window_size[1] > base_win_size[1]):
            assert window_size[0] % base_win_size[
                0] == 0, "please ensure the window size is smaller than or divisible by the base window size"
            assert window_size[1] % base_win_size[
                1] == 0, "please ensure the window size is smaller than or divisible by the base window size"
    ...
    def check_image_size(self, x, win_size):
        ...
        if mod_pad_h >= h or mod_pad_w >= w:
            ...
            # x = F.pad(x, (0, pad_w, 0, pad_h), 'replicate')
        else:
            pad_h, pad_w = 0, 0
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, base_win_size,
        ...
        # self.convs = nn.ModuleList([nn.Linear(dim * 2, dim)
        #                             for i in range((depth-1) // 2)])
        self.convs = nn.Linear(dim * 2, dim)
    ...
    def forward(self, x, x_size):
        ...
        for i, blk in enumerate(self.blocks):
            if i >= self.depth - (self.depth - 1) // 2:
                x = torch.cat([x, out[self.depth - i - 1]], dim=-1)
                # x = self.convs[i + (self.depth-1) // 2 - self.depth](x)
                x = self.convs(x)
...
class BVI_SRF(nn.Module, PyTorchModelHubMixin):
    """ HiT-SRF network.
    ...
    def __init__(self, img_size=96, patch_size=1, in_chans=3,
                 embed_dim=48, depths=[5], num_heads=[6],
                 ...
                 hier_win_ratios=[8, 4, 1, 4, 8],
                 **kwargs):
        super(BVI_SRF, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
